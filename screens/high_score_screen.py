...
class HighScoreScreen:
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        if self.bar_image:
            self.bar_image = pygame.transform.smoothscale(
                self.bar_image, (self.BAR_WIDTH, self.BAR_HEIGHT)
            )
        else:
    ...
    def _load_image(self, filename: str) -> pygame.Surface | None:
        ...
        try:
            ...
            if (
                ...
                and image.get_size() != self.screen.get_size()
            ):
    ...
    def _draw_empty_state(self) -> None:
        ...
        box_rect = pygame.Rect(
            self.START_X, self.START_Y + self.HEADER_HEIGHT + 40,
        ...
        pygame.draw.rect(
            transparent_surface, (28, 18, 48, 180),
            transparent_surface.get_rect(), border_radius=12
        )
        pygame.draw.rect(
            transparent_surface, self.HEADER_COLOR,
            transparent_surface.get_rect(), width=2, border_radius=12
        )
        self.screen.blit(
            transparent_surface, box_rect.topleft
        )

        msg_font = pygame.font.Font(None, 48)
        sub_font = pygame.font.Font(None, 32)
        msg_text = msg_font.render(
            "NO RECORDS YET", True, (255, 60, 170)
        )
        ...
        msg_rect = msg_text.get_rect(
            center=(box_rect.centerx, box_rect.centery - 15)
        )
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
            ...
            if total_items > self.VISIBLE_ITEMS:
                ...
                self.scroll_offset = max(
                    0, min(self.scroll_offset, max_scroll)
                )
            ...
            if sb_data and sb_data["thumb_rect"].collidepoint(event.pos):
                ...
                self.drag_mouse_offset_y = (
                    event.pos[1] - sb_data["thumb_rect"].y
                )
                return None
    ...
    def draw(self) -> None:
        ...
        if not self.high_scores:
            ...
            total_list_height = (
                self.BAR_HEIGHT + self.GAP_BETWEEN_BARS
            ) * self.VISIBLE_ITEMS
            clip_rect = pygame.Rect(
                self.START_X, self.START_Y + self.HEADER_HEIGHT,
                self.BAR_WIDTH, total_list_height
            )
    ...
    def _draw_scrollbar(self) -> None:
        ...
        thumb_color = (
            "#5e17eb" if self.is_dragging else self.HEADER_COLOR
        )
        pygame.draw.rect(
            self.screen, self.SCROLL_TRACK_COLOR,
            track_rect, border_radius=6
        )
        pygame.draw.rect(
            self.screen, thumb_color, thumb_rect, border_radius=6
        )
    ...
    def _draw_header(self) -> None:
        ...
        pygame.draw.rect(
            self.screen, self.HEADER_COLOR, header_rect,
            border_top_left_radius=8, border_top_right_radius=8
        )

        rank_surface = self.font_header.render(
            "RANK", True, self.TEXT_COLOR
        )
        name_surface = self.font_header.render(
            "NAME", True, self.TEXT_COLOR
        )
        score_surface = self.font_header.render(
            "TOTAL POINT", True, self.TEXT_COLOR
        )

        self.screen.blit(
            rank_surface, (self.START_X + 20, self.START_Y + 10)
        )
        self.screen.blit(
            name_surface, (self.START_X + 180, self.START_Y + 10)
        )
        self.screen.blit(
            score_surface, (
                self.START_X + self.BAR_WIDTH -
    ...
    def _draw_score_bars(self) -> None:
        ...
        start_drawing_y -= self.scroll_offset * (
            self.BAR_HEIGHT + self.GAP_BETWEEN_BARS
        )
        ...
        for index, player in enumerate(self.high_scores):
            ...
            if current_y + self.BAR_HEIGHT < self.START_Y + self.HEADER_HEIGHT:
                ...
                continue
            if current_y > self.START_Y + self.HEADER_HEIGHT + (
                (
                    self.BAR_HEIGHT + self.GAP_BETWEEN_BARS
                ) * self.VISIBLE_ITEMS
            ):
            ...
            if self.bar_image:
                self.screen.blit(
                    self.bar_image, (self.START_X, current_y)
                )
            rank_surface = self.font_rank.render(
                str(index + 1), True, self.TEXT_COLOR
            )
            name_surface = self.font_data.render(
                str(player.get("name", "Unknown")), True, self.TEXT_COLOR
            )
            score_surface = self.font_data.render(
                str(player.get("score", "0")), True, self.TEXT_COLOR
            )
            ...
            score_rect = score_surface.get_rect(
                midright=(
                    self.START_X + self.BAR_WIDTH - 40,
                    current_y + self.BAR_HEIGHT // 2
                )
            )
    ...
    def _get_scrollbar_data(self) -> dict | None:
        ...
        track_rect = pygame.Rect(
            self.SCROLLBAR_X, self.SCROLLBAR_START_Y,
            self.SCROLLBAR_WIDTH, total_visible_height
        )

        thumb_height_ratio = self.VISIBLE_ITEMS / total_items
        thumb_height = max(
            30, int(
                total_visible_height * thumb_height_ratio
            )
        )
        ...
        thumb_y = self.SCROLLBAR_START_Y + int(
