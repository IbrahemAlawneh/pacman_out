...
class SettingScreen:
    def __init__(self, screen: pygame.Surface, config: dict) -> None:
        ...
        """check if there is this keys and if not add it with default  values"""
        ...
        """here the images path so later if we need to change or customieze another theme or animations"""
        self.assets_path = Path("assets/main_menu_images/setting")
        self.bg_image = self._load_image("without_logo_bk.jpg", scale_to_screen=True)
        ...
        self.COLOR_PANEL = (88, 38, 175)     # بنفسجي
        self.COLOR_TEXT = (255, 255, 255)
        self.COLOR_ACCENT = (246, 135, 20)   # برتقالي
        self.COLOR_SLIDER_BG = (200, 200, 200)


        self._setup_hitboxes()

    def _load_image(self, filename: str, scale_to_screen: bool = False) -> pygame.Surface | None:
    ...
    def _play_music(self, index: int) -> None:
        """تشغيل مسار موسيقي بناءً على الزر المضغوط"""
        try:
    ...
    def _update_slider(self, slider_type: str, mouse_x: int) -> None:
        """تحويل مكان الماوس إلى نسبة مئوية للصوت"""
        rect = self.slider_music_rect if slider_type == "music" else self.slider_sfx_rect
    ...
    def draw(self) -> None:
        ...
        pygame.draw.rect(self.screen, self.COLOR_PANEL, panel_rect, border_radius=20)
        pygame.draw.rect(self.screen, self.COLOR_ACCENT, panel_rect, width=3, border_radius=20)
    ...
    def _draw_audio_section(self) -> None:
        # عناوين الصوت
        lbl_music = self.font_label.render("BACKGROUND MUSIC", True, self.COLOR_TEXT)
        lbl_sfx = self.font_label.render("SOUND EFFECT", True, self.COLOR_TEXT)
        self.screen.blit(lbl_music, (180, 240))
        self.screen.blit(lbl_sfx, (180, 340))
        
        if self.speaker_icon:
            scaled_speaker = pygame.transform.smoothscale(self.speaker_icon, (30, 30))
            self.screen.blit(scaled_speaker, (130, 235))
            self.screen.blit(scaled_speaker, (130, 335))


        pygame.draw.rect(self.screen, self.COLOR_SLIDER_BG, self.slider_music_rect, border_radius=5)
        fill_width = int(self.music_volume * self.slider_music_rect.width)
        pygame.draw.rect(self.screen, self.COLOR_ACCENT, (self.slider_music_rect.x, self.slider_music_rect.y, fill_width, 10), border_radius=5)
        pygame.draw.circle(self.screen, (255,220,180), (self.slider_music_rect.x + fill_width, self.slider_music_rect.centery), 10)

        pygame.draw.rect(self.screen, self.COLOR_SLIDER_BG, self.slider_sfx_rect, border_radius=5)
        fill_width_sfx = int(self.sfx_volume * self.slider_sfx_rect.width)
        pygame.draw.rect(self.screen, self.COLOR_ACCENT, (self.slider_sfx_rect.x, self.slider_sfx_rect.y, fill_width_sfx, 10), border_radius=5)
        pygame.draw.circle(self.screen, (255,220,180), (self.slider_sfx_rect.x + fill_width_sfx, self.slider_sfx_rect.centery), 10)
    ...
    def _draw_number_inputs(self) -> None:
        ...
        for label_text, key, x, y in settings:
            ...
            lbl = pygame.font.Font(None, 22).render(label_text, True, self.COLOR_TEXT)
            ...
            val_text = self.font_value.render(str(self.config[key]), True, (0, 0, 0))
            self.screen.blit(val_text, (x + 10, y + 38))
            

            if self.arrow_up and self.arrow_down:
                scaled_up = pygame.transform.smoothscale(self.arrow_up, (20, 15))
                scaled_down = pygame.transform.smoothscale(self.arrow_down, (20, 15))
    ...
    def _draw_cheat_section(self) -> None:
        ...
        for key, rect in self.cheat_toggles.items():
            ...
            if is_on and self.toggle_on:
                scaled_toggle = pygame.transform.smoothscale(self.toggle_on, (80, 40))
                self.screen.blit(scaled_toggle, rect.topleft)
            elif not is_on and self.toggle_off:
                scaled_toggle = pygame.transform.smoothscale(self.toggle_off, (80, 40))
                self.screen.blit(scaled_toggle, rect.topleft)
            else:

                pygame.draw.rect(self.screen, self.COLOR_ACCENT if is_on else (100,100,100), rect, border_radius=20)
