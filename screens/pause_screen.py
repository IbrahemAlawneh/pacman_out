...
class PauseScreen:
    ...
    def __init__(self, surface: pygame.Surface) -> None:
        ...
        labels = (
            ("RESUME GAME", "resume"), ("HOW TO PLAY", "instructions"),
    ...
    def _text(
        self, value: str, pos: tuple[int, int],
        ...
        self.surface.blit(
            ...
            else image.get_rect(topleft=pos)
        )
    ...
    def _draw_button(self, rect: pygame.Rect, label: str, active: bool) -> None:
        ...
        gloss = pygame.Surface(
            (rect.width - 16, rect.height // 3), pygame.SRCALPHA
        )
        pygame.draw.rect(
            gloss, (255, 255, 255, 65), gloss.get_rect(), border_radius=12
        )
        self.surface.blit(gloss, (rect.x + 8, rect.y + 4))
        pygame.draw.rect(
            self.surface, border, rect, 3 if active else 2, border_radius=32
        )
        ...
        if active:
            ...
            pygame.draw.rect(
                glow, (*border, 45), glow.get_rect(), border_radius=40
            )
    ...
    def _draw_menu(self) -> None:
        ...
        self._text(
            "THE GAME IS ON HOLD", (panel.centerx, panel.y + 93),
            self.small, self.MUTED
        )
        pygame.draw.line(
            self.surface, self.LINE, (panel.x + 45, panel.y + 118),
            (panel.right - 45, panel.y + 118)
            )
    ...
    def _draw_help(self) -> None:
        ...
        self._text(
            "CONTROLS & CHEAT COMMANDS", (panel.centerx, panel.y + 82),
            self.small, self.CYAN
        )
        self._text(
            "ENABLE IN SETTINGS FIRST", (panel.centerx, panel.y + 104),
            self.small, self.PURPLE
        )
        pygame.draw.line(
            self.surface, self.LINE,
            (panel.x + 40, panel.y + 125), (panel.right - 40, panel.y + 125)
        )

        rows = (
            ("MOVE", "Arrow keys or WASD", self.CYAN),
            ("SKIP LEVEL", "F1", self.PURPLE),
            ("FREEZE GHOSTS", "F2", self.PURPLE),
            ("EXTRA LIFE", "F3", self.PURPLE),
            ("SPEED BOOST", "F4", self.PURPLE),
            ("GHOST SHIELD", "F5", self.PURPLE)
        )
        ...
        for i, (name, key, color) in enumerate(rows):
            ...
            name_box = pygame.Rect(
                panel.x + 42, y - 5, name_img.get_width() + 26, 30
            )
            key_img = self.text.render(key, True, self.WHITE)
            key_box = pygame.Rect(
                panel.right - key_img.get_width() - 68, y - 5,
                key_img.get_width() + 26, 30
            )
            for box, fill, border in (
                (name_box, (19, 31, 65, 170), color),
                (key_box, (19, 31, 65, 200), self.CYAN)
            ):
                layer = pygame.Surface(box.size, pygame.SRCALPHA)
                pygame.draw.rect(
                    layer, fill, layer.get_rect(), border_radius=9
                )
                self.surface.blit(layer, box.topleft)
                pygame.draw.rect(
                    self.surface, border, box, 1, border_radius=9
                )

            self.surface.blit(
                name_img, name_img.get_rect(center=name_box.center)
            )
            self.surface.blit(
                key_img, key_img.get_rect(center=key_box.center)
            )
            pygame.draw.line(
                self.surface, self.LINE,
                (panel.x + 40, y + 31), (panel.right - 40, y + 31)
            )
        self._text(
            "ESC / BACKSPACE  RETURN TO PAUSE MENU",
            (panel.centerx, panel.bottom - 25), self.small, self.MUTED
        )
