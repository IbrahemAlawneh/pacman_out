...
class GameResult:
    ...
    def __init__(self, screen: pygame.Surface, won: bool, score: int, next_level: int | None = None, file_name = "high_score.json") -> None:
        ...
        self.fonts = {s: pygame.font.Font(font, s) for s in (56, 30, 24, 20, 16)}
    ...
    def handle_event(self, event: pygame.event.Event) -> str | None:
        ...
        if self.stage == "enter_name" and event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE: self.stage = "result"
            elif event.key == pygame.K_RETURN:
                self._save_score(self.name_input.strip())
                self.saved_message = f'SAVED AS "{self.name_input.strip() or "unknown"}"'
                self.stage = "result"
            elif event.key == pygame.K_BACKSPACE: self.name_input = self.name_input[:-1]
            elif len(self.name_input) < 10 and (event.unicode.isalnum() or event.unicode == " "): self.name_input += event.unicode
            return None
        
        if self.stage == "result":
            if event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_UP, pygame.K_w): self.selected = 0
                elif event.key in (pygame.K_DOWN, pygame.K_s): self.selected = 1
                elif event.key in (pygame.K_RETURN, pygame.K_KP_ENTER): return self.btns[self.selected]["action"]
            elif event.type == pygame.MOUSEMOTION and event.rel != (0, 0):
                for i, b in enumerate(self.btns): 
                    if b["rect"].collidepoint(event.pos): self.selected = i
    ...
    def update(self) -> str | None:
        if self.stage == "transition" and pygame.time.get_ticks() - self.started_at >= 1000: return "next_level"
        return None

    def _text(self, txt: str, pos: tuple, size: int, color: tuple | None = None, bold: bool = False) -> None:
        img = (self.fonts["bold"] if bold else self.fonts[size]).render(txt, True, color or self.WHITE)
        self.screen.blit(img, img.get_rect(center=pos))

    def _panel(self, w: int, h: int, border: tuple) -> pygame.Rect:
        r = pygame.Rect(0, 0, w, h); r.center = self.screen.get_rect().center
        fill = pygame.Surface(r.size, pygame.SRCALPHA)
        pygame.draw.rect(fill, self.PANEL_FILL, fill.get_rect(), border_radius=22)
        self.screen.blit(fill, r.topleft)
        pygame.draw.rect(self.screen, border, r, width=2, border_radius=22)
        return r

    def draw(self) -> None:
        overlay = pygame.Surface(self.screen.get_size(), pygame.SRCALPHA)
        overlay.fill(self.OVERLAY); self.screen.blit(overlay, (0, 0))
        ...
        if self.stage == "transition":
            ...
            for off, col in ((4, self.DIM), (2, self.PINK), (0, self.WHITE)):
                self._text("★ LEVEL COMPLETE ★", (cx + off, p.y + 62 + off), 30, col)
            ...
            pygame.draw.rect(self.screen, self.CYAN, box, width=2, border_radius=10)
            self._text(self.name_input or "TYPE YOUR NAME", box.center, 20, self.PINK if self.name_input else self.MUTED)
            self._text("ENTER TO SAVE   ESC TO SKIP", (cx, p.bottom - 35), 16, self.MUTED)
            
        else:
            p = self._panel(480, 430, self.PINK if self.won else self.RED)
            self._text("YOU WIN!" if self.won else "GAME OVER", (cx, p.y + 60), 56, self.GOLD if self.won else self.RED, not self.won)
            self._text(f"FINAL SCORE: {self.score}", (cx, p.y + 112), 30)
            if self.saved_message: self._text(self.saved_message, (cx, p.y + 150), 16, self.CYAN)
            pygame.draw.line(self.screen, self.DIM, (p.x + 48, p.y + 178), (p.right - 48, p.y + 178))
            ...
            for i, b in enumerate(self.btns):
                ...
                pygame.draw.rect(fill, (*border, 60) if active else (*self.DIM, 130), fill.get_rect(), border_radius=14)
                self.screen.blit(fill, r.topleft)
                
                pygame.draw.rect(self.screen, border, r, width=3 if active else 1, border_radius=14)
                self._text(b["text"], r.center, 24, self.WHITE if active else self.MUTED)
            self._text("\u2191 \u2193 SELECT   ENTER CONFIRM", (cx, p.bottom - 25), 16, self.MUTED)
