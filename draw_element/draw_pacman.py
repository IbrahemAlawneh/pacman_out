...
class DrawPacman:
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        self.img_close = pygame.image.load(
            os.path.join(img_path, "close.png")
        ).convert_alpha()
        self.img_open = pygame.image.load(
            os.path.join(img_path, "open.png")
        ).convert_alpha()
    ...
    def draw(
        ...
        if self.last_cell_size != cell_size and cell_size > 0:
            ...
            self.scaled_close = pygame.transform.scale(
                self.img_close, (size, size)
            )
            self.scaled_open = pygame.transform.scale(
                self.img_open, (size, size)
            )
