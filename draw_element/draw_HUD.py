...
class DrawHUD:
    ...
    def __init__(self, font_path: str):
        ...
        raw_on = pygame.image.load(
            os.path.join(themes_path, "on.png")
        ).convert_alpha()
        raw_off = pygame.image.load(
            os.path.join(themes_path, "off.png")
        ).convert_alpha()
    ...
    def draw(
        ...
        lives_surface = self.font.render(
            f"x{lives}", False, self.text_color
        )
        score_surface = self.font_sec.render(
            f"{score} PTS", False, self.text_color
        )
        level_surface = self.font_sec.render(
            f"{level:02d} LEVEL", False, self.text_color
        )
        ...
        for i, (name, state) in enumerate(cheats):
            ...
            text_surface = self.font_cheat.render(
                name, False, self.text_color
            )
            text_rect = text_surface.get_rect(
                center=(
                    x_pos + (current_img.get_width() // 2),
                    y_pos + current_img.get_height() + 12
                )
            )
